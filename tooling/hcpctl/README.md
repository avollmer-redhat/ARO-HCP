...
Without this flag, logs from all pods of the same container are written to a single file (e.g. `cluster-namespace-container.jsonl`). With `--split-by-pod`, each pod gets its own file (e.g. `cluster-namespace-container-pod.jsonl`).
