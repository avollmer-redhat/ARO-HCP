...
func TestCliOutputFunc_SplitByPod(t *testing.T) {
	...
	fileA := filepath.Join(tempDir, "services", "cluster1-default-frontend-frontend-abc123.jsonl")
	fileB := filepath.Join(tempDir, "services", "cluster1-default-frontend-frontend-def456.jsonl")
...
func TestCliOutputFunc_SplitByPod_EmptyPodName(t *testing.T) {
	...
	expectedFile := filepath.Join(tempDir, "services", "cluster1-default-backend.jsonl")
	assert.FileExists(t, expectedFile)
}
...
func TestCliOutputFunc_SplitByPod_Disabled(t *testing.T) {
	...
	expectedFile := filepath.Join(tempDir, "services", "cluster1-default-frontend.jsonl")
	assert.FileExists(t, expectedFile)
	content, err := os.ReadFile(expectedFile)
	require.NoError(t, err)
	lines := splitNonEmpty(string(content))
