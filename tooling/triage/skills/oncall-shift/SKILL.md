...
1. **Report page** (`/report`, DEV env) — watch the E2E success card (after
   the last push of a merged PR). This is the key metric. A dropping value
   is your alarm.

2. **Run Log** (`/run-log?date=<YYYY-MM-DD>&env=dev`) — type `batch` in the
...
3. **Failure Patterns** (`/failure-patterns`) — this is a *triage* tool, not
   a monitoring tool. Use it to rank the most impactful issues by run-impact
...
- **Timeout patterns**: Patterns like `CreateHCPClusterAndWait`, `context
  deadline exceeded`, etc. are generic timeouts hiding distinct root causes.
  Occurrence count alone is meaningless. Timeouts require deeper analysis
  from Kusto logs using `hcpctl snapshot analyze` (in `tooling/hcpctl`)
